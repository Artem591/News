export default async (policyContext, config, { strapi }) => {
  ...
  const foundUser = await strapi.entityService.findOne('plugin::users-permissions.user', user.id, {
    populate: ['role'],
  });
  ...
  if (isEditor || isAuthor) {
    strapi.log.info(
      `User ${user.id} (${foundUser.email}) ${isEditor ? 'as Editor' : `as Author to article ${id} "${article.title}`}`
    );
    return true;
  } else {
