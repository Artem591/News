...
export default factories.createCoreController('api::article.article', ({ strapi }) => ({
    ...
    async publish(ctx) {
        ...
        try {
            ...
            strapi.log.info(`Article published: ID ${id}, Title "${article.title}", User ${user.id} (${user.email})`);
    ...
    async delete(ctx) {
        ...
        try {
            ...
            strapi.log.info(`Deleted ${result.count} article versions with document_id: ${article.documentId}`);
